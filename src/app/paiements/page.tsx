        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6 md:space-y-10">
          ...
          <TabsContent value="dashboard" className="space-y-6 md:space-y-10 animate-in slide-in-from-bottom-4">
            ...
            <div className="grid lg:grid-cols-12 gap-6 md:gap-10">
              <Card className="lg:col-span-8 border-none shadow-sm bg-white rounded-[2rem] md:rounded-[3.5rem] p-6 md:p-12">
                 <div className="flex items-center justify-between mb-8 md:mb-14">
                    ...
                    <Button variant="ghost" className="font-black text-primary text-[9px] md:text-sm rounded-xl h-9 md:h-12 hover:bg-primary/5 transition-all">VOIR TOUT <ChevronRight className="ml-1 size-3" /></Button>
                 </div>
                 
                 <div className="space-y-3">
                    {loadingPayments ? (
                      <div className="py-20 text-center animate-pulse"><Loader2 className="animate-spin mx-auto text-primary/10 size-8 md:size-10" /></div>
                    ) : (payments?.length === 0 && expenses?.length === 0) ? (
                      <div className="py-20 text-center space-y-6 opacity-30">
                        <HardDrive className="size-12 md:size-16 mx-auto text-muted-foreground" />
                        <p className="italic font-medium text-xs md:text-xl">Aucune donnée certifiée pour l'instant.</p>
                      </div>
                    ) : (
                                ...
                                <div className="flex items-center gap-4 md:gap-8">
                                   ...
                                   <div className="min-w-0">
                                      ...
                                      <div className="flex items-center gap-3">
                                        <Badge variant="outline" className={cn("text-[6px] md:text-[10px] font-black uppercase px-2 md:px-4 py-0.5 border-2 rounded-full", isExpense ? "border-red-100 text-red-600" : "border-emerald-100 text-emerald-600")}>
                                          {isExpense ? tx.category : tx.description}
                                        </Badge>
                                        <span className="text-[6px] md:text-[9px] font-bold text-muted-foreground uppercase hidden sm:inline-block">Scellé le {new Date(tx.date).toLocaleDateString('fr-FR')}</span>
                                      </div>
                                   </div>
                                </div>
                                <div className="text-right">
                                   <p className={cn("font-black text-sm md:text-2xl tabular-nums", isExpense ? "text-red-600" : "text-emerald-600")}>
                                     {isExpense ? '-' : '+'}{Number(isExpense ? tx.amount : tx.amountPaid).toLocaleString()} F
                                   </p>
                                   <span className="text-[6px] md:text-[10px] font-black text-muted-foreground uppercase opacity-40 sm:hidden">{new Date(tx.date).toLocaleDateString('fr-FR')}</span>
                                </div>
                              </div>
                            )
                          })
              ...
              <div className="lg:col-span-4 space-y-6 md:gap-10">
                <Card className="p-7 md:p-12 rounded-[2.5rem] md:rounded-[3.5rem] bg-primary/5 border-2 border-dashed border-primary/20 group hover:bg-primary/10 transition-all relative overflow-hidden">
                   ...
                   <Zap className="absolute -bottom-10 -left-10 size-32 md:size-40 text-primary/5 pointer-events-none group-hover:scale-110 transition-transform duration-1000" />
                </Card>
          ...
          <TabsContent value="encaissements" className="animate-in fade-in">
             <Card className="border-none shadow-sm bg-white rounded-[2rem] md:rounded-[4rem] overflow-hidden min-h-[500px]">
                <div className="p-6 md:p-12 border-b bg-muted/5 flex flex-col md:flex-row md:items-center justify-between gap-6">
                   <div className="space-y-1">
                      <h3 className="text-lg md:text-3xl font-black tracking-tight uppercase">Journal des Recettes</h3>
                   ...
                   <div className="relative group w-full md:w-80">
                      ...
                      <Input 
                        ...
                        className="pl-12 h-11 md:h-14 rounded-2xl bg-white border-2 font-bold text-xs md:text-sm"
                      />
                   </div>
                </div>
          ...
          <TabsContent value="depenses" className="animate-in fade-in">
             <div className="flex flex-col md:flex-row justify-between items-center bg-white p-6 md:p-10 rounded-[2.2rem] md:rounded-[3.5rem] shadow-sm gap-6 mb-8 md:mb-12 border-2 border-primary/5 relative overflow-hidden group">
                <div className="absolute top-0 left-0 p-8 opacity-[0.02] pointer-events-none group-hover:scale-110 transition-transform duration-1000"><PiggyBank className="size-24 md:size-32" /></div>
                <div className="text-center md:text-left relative z-10">
                   <h3 className="text-lg md:text-3xl font-black tracking-tight uppercase">Pilotage des Sorties</h3>
                   <p className="text-[8px] md:text-sm font-medium text-muted-foreground uppercase tracking-widest mt-1">Contrôle de l'équilibre budgétaire</p>
                </div>
