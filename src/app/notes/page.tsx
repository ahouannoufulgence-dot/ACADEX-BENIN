    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (configSnap.exists()) {
          ...
          const def = (userSubject === 'Mathématiques' || userSubject === 'Français') ? 4 : 2
          setClassCoefficient(def)
        }
      ...
      <div className="space-y-6 md:space-y-10 animate-in fade-in duration-500">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-1.5">
            ...
            <div className="flex items-center gap-3 text-muted-foreground font-bold text-[10px] md:text-sm">
              ...
              <span>Année {activeYear} • Scellage Institutionnel</span>
            </div>
          </div>
          <Button 
        ...
        <div className="grid lg:grid-cols-12 gap-6 md:gap-10">
          <div className="lg:col-span-9 space-y-6 md:space-y-10">
            <Card className="border-none shadow-sm bg-white rounded-[2rem] md:rounded-[2.5rem] p-5 md:p-10 border-l-[8px] md:border-l-[15px] border-primary">
              <div className={cn("grid gap-4 md:gap-8", isDirector ? "grid-cols-2 lg:grid-cols-5" : "grid-cols-2 md:grid-cols-4")}>
                <div className="space-y-1.5">
                  ...
                  <Select onValueChange={setSelectedClass} value={selectedClass}>
                    <SelectTrigger className="h-11 md:h-14 rounded-2xl border-2 font-black text-xs md:text-base"><SelectValue placeholder="Choisir" /></SelectTrigger>
                ...
                {isDirector && (
                  <div className="space-y-1.5">
                    ...
                    <Select value={userSubject} onValueChange={setUserSubject}>
                       <SelectTrigger className="h-11 md:h-14 rounded-2xl border-2 font-black text-xs md:text-base"><SelectValue placeholder="Sujet" /></SelectTrigger>
                       <SelectContent className="rounded-2xl border-2 p-1">
                ...
                <div className="space-y-1.5">
                  ...
                  <Select value={selectedTrimestre} onValueChange={setSelectedTrimestre}>
                    <SelectTrigger className="h-11 md:h-14 rounded-2xl border-2 font-black text-xs md:text-base"><SelectValue /></SelectTrigger>
                    <SelectContent className="rounded-2xl border-2 p-1">
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    <Input type="number" min="1" max="10" value={classCoefficient} onChange={(e) => setClassCoefficient(Number(e.target.value))} className="h-11 md:h-14 rounded-2xl border-2 font-black text-center text-sm md:text-xl shadow-inner focus-visible:ring-primary" />
            ...
            {selectedClass && (
              <Card className="border-none shadow-sm bg-white rounded-[2.2rem] md:rounded-[3.5rem] overflow-hidden">
                <div className="p-6 md:p-12 border-b bg-muted/5 flex flex-col md:flex-row md:items-center justify-between gap-4">
                  ...
                  {loadingExisting && <div className="flex items-center gap-2 text-[9px] md:text-[11px] font-black text-primary animate-pulse uppercase tracking-widest"><RefreshCw className="size-2.5 md:size-4 animate-spin" /> Synchronisation...</div>}
                </div>
                    ...
                    <table className="w-full">
                      <thead className="bg-muted/20 text-[10px] font-black uppercase text-muted-foreground border-b border-muted/30">
                        <tr>
                          ...
                          <th className="px-12 py-8 text-right bg-primary text-white tracking-widest">Moyenne Coefficiée</th>
                        </tr>
                      </thead>
          ...
          <div className="lg:col-span-3 space-y-6 md:space-y-10">
            <Card className="p-7 md:p-9 rounded-[2rem] md:rounded-[2.5rem] bg-amber-50 border-2 border-amber-100 flex flex-col gap-6 relative overflow-hidden group">
              <div className="flex items-center gap-3 text-amber-700 relative z-10">
                ...
                <h4 className="font-black text-[10px] md:text-sm uppercase tracking-widest">Architecture Scellée</h4>
              </div>
              <p className="text-[10px] md:text-xs font-medium leading-relaxed text-amber-800 relative z-10">
                Les coefficients sont récupérés dynamiquement depuis le module <b>Matières</b>. La <b>Conduite</b> de l'élève impacte automatiquement sa moyenne générale finale dans le module <b>Promotions</b>.
              </p>
              <Button asChild variant="outline" className="w-full rounded-xl border-amber-200 text-amber-700 font-black text-[9px] md:text-xs bg-white hover:bg-amber-100 h-11 md:h-13 transition-all relative z-10">
                <Link href="/promotions">Voir Classements <ArrowRight className="ml-2 size-3" /></Link>
              </Button>
            </Card>
            ...
            <Card className="p-8 rounded-[2.5rem] bg-foreground text-white border-none shadow-2xl relative overflow-hidden flex flex-col items-center text-center gap-4">
               ...
               <div>
                  <h4 className="font-black text-sm uppercase tracking-widest">Intelligence Acadex</h4>
                  <p className="text-[9px] font-medium text-white/40 mt-1 uppercase">Calculs de moyennes certifiés</p>
               </div>
            </Card>
          </div>
        </div>
