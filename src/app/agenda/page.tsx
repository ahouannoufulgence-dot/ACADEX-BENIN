      ...
      <div className="space-y-6 md:space-y-10 animate-in fade-in duration-500">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 px-1">
          ...
          <div className="flex gap-2 w-full md:w-auto">
            ...
            <Badge className="bg-primary text-white h-11 md:h-14 px-5 md:px-8 rounded-xl md:rounded-2xl flex items-center gap-2 md:gap-3 font-black text-[9px] md:text-sm shadow-xl shadow-primary/20">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6 md:space-y-10">
          <TabsList className="bg-white border-2 rounded-[1.4rem] md:rounded-[2.5rem] h-12 md:h-20 p-1.5 flex w-full md:w-fit shadow-md overflow-x-auto no-scrollbar scroll-smooth">
            {[
          ...
          {/* Contrôles de vue */}
          <Card className="p-4 md:p-8 rounded-[1.8rem] md:rounded-[3rem] bg-white border-none shadow-sm flex flex-col md:flex-row items-center gap-4 md:gap-10">
                ...
                <div className="flex gap-1.5 overflow-x-auto no-scrollbar pb-1">
                   {days.map(d => (
                     <button 
                       ...
                       className={cn(
                         "px-4 md:px-6 py-2 md:py-3 rounded-xl font-black text-[9px] md:text-[11px] uppercase transition-all whitespace-nowrap border-2",
             ...
             {activeTab === 'professeur' && (
                <div className="w-full md:w-64">
                   ...
                   <Select value={selectedTeacher} onValueChange={setSelectedTeacher}>
                      ...
                      <SelectContent className="rounded-xl border-2 p-1.5 max-h-[300px]">
                         {teachers?.map((t:any) => <SelectItem key={t.id} value={t.officialId} className="font-bold p-2.5 rounded-lg text-xs">{t.fullName}</SelectItem>)}
                      </SelectContent>
                   </Select>
                </div>
             )}
          ...
          <TabsContent value="global" className="animate-in zoom-in-95">
             <Card className="border-none shadow-sm bg-white rounded-[1.8rem] md:rounded-[3.5rem] overflow-hidden">
                <div className="p-5 md:p-12 border-b bg-muted/5 flex flex-col md:flex-row md:items-center justify-between gap-5">
                   <div className="space-y-1">
                      <h3 className="text-xl md:text-3xl font-black uppercase tracking-tight">Omni-Vision <span className="text-primary italic">Directe</span></h3>
                      <p className="text-[7px] md:text-xs font-bold text-muted-foreground uppercase tracking-widest">Flux temporel synchronisé sur l'établissement</p>
                   </div>
                   <Badge variant="outline" className="h-8 md:h-11 rounded-full border-2 border-primary/20 text-primary font-black px-4 text-[8px] md:text-xs uppercase">MATRICE LIVE</Badge>
                </div>
                ...
                <div className="overflow-x-auto p-2 md:p-6 no-scrollbar">
                   <table className="w-full border-separate border-spacing-1 md:border-spacing-3">
                      <thead>
                         <tr>
                            <th className="p-2 md:p-6 bg-muted/30 rounded-lg md:rounded-2xl text-[7px] md:text-[10px] font-black uppercase text-muted-foreground tracking-[0.2em] min-w-[70px]">HORAIRE</th>
                            {OFFICIAL_CLASSES.map(c => (
                              <th key={c} className="p-2 md:p-6 bg-primary/5 rounded-lg md:rounded-2xl text-[8px] md:text-sm font-black uppercase text-primary border-2 border-primary/10 min-w-[90px] md:min-w-[150px]">{c}</th>
                            ))}
                         </tr>
                      </thead>
                      <tbody>
                         {timeSlots.map(time => (
                           <tr key={time}>
                              <td className="p-2 md:p-6 bg-muted/20 rounded-lg md:rounded-2xl text-center font-black text-[9px] md:text-2xl tabular-nums text-foreground/40">{time}</td>
                              {OFFICIAL_CLASSES.map(cls => {
                                ...
                                return (
                                  <td key={`${cls}-${time}`} className={cn(
                                    "p-1 md:p-4 rounded-lg md:rounded-2xl transition-all border-2 h-14 md:h-28 text-center",
                                    course ? "bg-white border-primary/10 shadow-lg scale-[1.02] ring-4 ring-primary/5" : "bg-muted/5 border-transparent opacity-[0.15]"
                                     ...
                                     {course ? (
                                       <div className="space-y-0.5 md:space-y-2 group cursor-help">
                                          <p className="text-[7px] md:text-base font-black text-foreground uppercase tracking-tight truncate group-hover:text-primary transition-colors">{course.subject}</p>
                                          <p className="text-[5px] md:text-[10px] font-bold text-muted-foreground uppercase truncate opacity-70">{course.teacherName.split(' ')[0]}</p>
                                          <Badge className="bg-primary/5 text-primary text-[5px] md:text-[8px] h-3 md:h-5 rounded-md border-none uppercase px-1.5">{course.room || '---'}</Badge>
                                       </div>
                                     ) : (
                                       <div className="flex items-center justify-center opacity-10"><ArrowRight className="size-2.5 md:size-5" /></div>
                                     )}
                                  </td>
                                )
                              })}
             ...
             <div className="grid lg:grid-cols-12 gap-6 md:gap-10">
                <div className="lg:col-span-8 space-y-4">
                   <div className="flex items-center justify-between px-2">
                      <h2 className="text-lg md:text-3xl font-black uppercase tracking-tight">{selectedClass} — <span className="text-primary italic">Programme</span></h2>
                      <Badge className="bg-primary text-white font-black px-3 py-1 rounded-full text-[8px] md:text-xs uppercase">{selectedDay}</Badge>
                   </div>
                   ...
                   <div className="grid gap-3 md:gap-6">
                      {loadingSchedules ? (
                        ...
                        <Card className="py-24 text-center border-4 border-dashed rounded-[2rem] md:rounded-[3.5rem] bg-white/50 opacity-30 flex flex-col items-center justify-center gap-4">
                        ...
                        filteredSchedules.filter((s:any) => s.day === selectedDay).map((course: any, i: number) => (
                          <Card key={i} className="p-4 md:p-8 bg-white rounded-2xl md:rounded-[3rem] border-none shadow-sm hover:shadow-xl transition-all group flex items-center justify-between gap-4">
                             <div className="flex items-center gap-3 md:gap-10 min-w-0">
                                <div className="size-11 md:size-24 bg-muted rounded-xl md:rounded-[2.5rem] flex flex-col items-center justify-center group-hover:bg-primary group-hover:text-white transition-all shrink-0 shadow-inner">
                                ...
                                <div className="space-y-0.5 md:space-y-1.5 truncate">
                                   <h4 className="text-sm md:text-3xl font-black text-foreground uppercase tracking-tight truncate group-hover:text-primary transition-colors">{course.subject}</h4>
                                   <div className="flex flex-wrap items-center gap-3 md:gap-6 text-[7px] md:text-[11px] font-bold text-muted-foreground uppercase tracking-widest">
                                      <span className="flex items-center gap-1.5"><User className="size-2.5 md:size-4 text-primary" /> {course.teacherName}</span>
                                      <span className="flex items-center gap-1.5"><MapPin className="size-2.5 md:size-4 text-primary" /> {course.room || 'Salle libre'}</span>
                                   </div>
                                </div>
                             </div>
                             <div className="text-right shrink-0">
                                <p className="text-[6px] md:text-[10px] font-black uppercase text-muted-foreground tracking-widest mb-0.5">Durée</p>
                ...
                <div className="lg:col-span-4 space-y-6">
                   <Card className="p-6 md:p-10 bg-foreground text-white rounded-[1.8rem] md:rounded-[3.5rem] shadow-2xl relative overflow-hidden group border-none">
                      <div className="relative z-10 space-y-6 md:space-y-10">
                         <div className="flex items-center gap-4">
                            <div className="size-10 md:size-16 bg-primary/20 rounded-2xl flex items-center justify-center shadow-inner"><Sparkles className="size-5 md:size-8 text-primary animate-pulse" /></div>
                            <h3 className="text-lg md:text-2xl font-black uppercase tracking-tight">Audit IA Brain</h3>
                         </div>
                         <div className="p-5 md:p-8 bg-white/5 rounded-xl md:rounded-[2rem] border border-white/10 italic text-[9px] md:text-base font-medium leading-relaxed text-white/80 min-h-[100px] flex items-center justify-center text-center">
                           {aiReport ? `"${aiReport}"` : `"Je peux analyser l'équilibre temporel de vos classes et détecter les conflits de salles."`}
                         </div>
                         <Button onClick={handleAiAudit} disabled={analyzing} className="w-full h-11 md:h-16 rounded-xl md:rounded-2xl bg-primary hover:bg-primary/90 text-white font-black text-[10px] md:text-lg transition-all active:scale-95 shadow-xl shadow-primary/20 uppercase">
